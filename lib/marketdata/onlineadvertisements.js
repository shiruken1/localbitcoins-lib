...
OnlineAdvertisements.prototype.getUri = function(options) {
    ...
    } else if (options.hasOwnProperty('countrycode') && options.countrycode) {
        const country = countries[options.countrycode].split('').map(l => { if(l === ' ') { return '-'; } else { return l.toLowerCase(); } }).join('');
        returnStr += options.countrycode.toLowerCase() + '/' + country;
